...
The first step is to linearize the diffeomorphisms $\theta(t)$ and $\nu(t)$ over the support of the first-order wavelet $\psi_{\lambda_1}(t)$. 

Around the $p^\text{th}$

To capitalize on these two identities, we factorize the scalogram
$x_{1}$ as the product of the scalograms $e_{1}$ and $h_{1}$, translated
along the log-frequency axis at respective velocities $\log_{2}\dot{\theta}(t)$
and $\log_{2}\dot{\nu}(t)$: 

\begin{equation}
e_{\theta,1}(t,\log_2 \lambda_1) = e_{1}(\log_2 \lambda_1 - \log_2 \dot{\theta}(t)), and
\end{equation}

\begin{equation}
h_{\nu,1}(t,\log_2 \lambda_1) = h_{1}(\log_2 \lambda_1 - \log_2 \dot{\nu}(t)).
\end{equation}

This factorization is achieved by linearizing $\theta(t)$ and $\nu(t)$
over the support of the first-order wavelet, and by converting the
local dilation of the signal into a dilation of the wavelet by means
of an affine change of variables. 
...
\item smooth spectral envelope: $\Vert\dot{\hat{h}}/\hat{h}\Vert_{\infty}\Vert1/\dot{\nu}\Vert_{\infty}\ll Q/\lambda_{1}$.
\end{itemize}

Additionally, the scalogram $x_1$ of the deformed source-filter model gets factorized as a product between the scalograms of the deformed source and the deformed filter:

\begin{equation}
x_1 = e_{\theta,1} \times h_{\nu,1}.
\label{eq:factorization}
\end{equation}

All wavelets are designed to carry a negligible mean value, i.e. convolving
them with a constant yields zero. Therefore, the harmonicity and spectral smothness properties rewrite as
...
Gathering Equations \ref{eq:factorization} and \ref{eq:zero-mean} into Equation \ref{eq:x2-joint} yields:
