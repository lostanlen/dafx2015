...
In order to disentangle variabilities in pitch and spectral envelope, we extend the joint time-frequency scattering transform to encompass motion across octaves, in conjuntion with motion along neighbouring constant-Q bands.
...
\begin{equation}
x(t) = \left[ e_{\theta}(t) \ast h_{\nu}(t) \right]
\label{fig:source-filter-definition}
\end{equation}

In this section, we show that second-order spiral coefficients $x_2$ linearize the derivatives of the diffeomorphisms $\theta(t)$ and $\tau(t)$.
